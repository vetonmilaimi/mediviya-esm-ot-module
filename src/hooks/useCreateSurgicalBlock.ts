 ...
 */
export function useCreateSurgicalBlock() {
  const createSurgicalBlock = async (url: string, { arg }: { arg: SurgicalBlockPayload }) => {
  ...
  const { trigger, isMutating, error } = useSWRMutation(
    '/ws/rest/v1/surgicalBlock',
    createSurgicalBlock
  );

  return {
    createSurgicalBlock: trigger,
